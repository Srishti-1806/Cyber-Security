...
import { Globe, Filter, Maximize2 } from "lucide-react"
...
export function FullScreenThreatMap() {
  ...
  useEffect(() => {
    ...
    const generateRandomAttacks = () => {
      ...
      setAttacks((prev) => {
        const updated = [...prev, ...newAttacks].slice(-200)
        ...
        updated.forEach((attack) => {
          if (!countryCounts[attack.source.country]) {
            countryCounts[attack.source.country] = 0
          }
          countryCounts[attack.source.country]++
        })
    ...
    const interval = setInterval(generateRandomAttacks, 2000)
          ...
          <Tabs defaultValue="map">
            ...
            <TabsContent value="map" className="m-0">
              <div className="relative h-[500px] w-full overflow-hidden">
                ...
                <div className="absolute bottom-4 left-4 rounded bg-background/80 p-3 backdrop-blur-sm">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    <Badge variant="outline" className="text-[10px]">
                      Live Attacks: {stats.total}
                    </Badge>
                    <Badge variant="outline" className="bg-red-500/10 text-[10px] text-red-500">
                      Malware: {stats.malware}
                    </Badge>
                    <Badge variant="outline" className="bg-amber-500/10 text-[10px] text-amber-500">
                      DDoS: {stats.ddos}
                    </Badge>
                  </div>
                </div>
            ...
            <TabsContent value="stats" className="m-0 p-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-4">
                  <h3 className="text-sm font-medium">Attack Types</h3>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-lg border border-border/50 p-3">
                      <div className="text-xs text-muted-foreground">Malware</div>
                      <div className="mt-1 text-2xl font-bold">{stats.malware}</div>
                      <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-secondary">
                        <div
                          className="h-full bg-red-500"
                          style={{ width: `${(stats.malware / stats.total) * 100}%` }}
                        />
                      </div>
                    </div>
                    <div className="rounded-lg border border-border/50 p-3">
                      <div className="text-xs text-muted-foreground">DDoS</div>
                      <div className="mt-1 text-2xl font-bold">{stats.ddos}</div>
                      <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-secondary">
                        <div
                          className="h-full bg-amber-500"
                          style={{ width: `${(stats.ddos / stats.total) * 100}%` }}
                        />
                      </div>
                    </div>
                    <div className="rounded-lg border border-border/50 p-3">
                      <div className="text-xs text-muted-foreground">Phishing</div>
                      <div className="mt-1 text-2xl font-bold">{stats.phishing}</div>
                      <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-secondary">
                        <div
                    ...
                    <div className="rounded-lg border border-border/50 p-3">
                      ...
                      <div className="mt-1 text-2xl font-bold">{stats.ransomware}</div>
                      <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-secondary">
                        <div
                          className="h-full bg-purple-500"
                          style={{ width: `${(stats.ransomware / stats.total) * 100}%` }}
                        />
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <h3 className="text-sm font-medium">Top Attack Sources</h3>
